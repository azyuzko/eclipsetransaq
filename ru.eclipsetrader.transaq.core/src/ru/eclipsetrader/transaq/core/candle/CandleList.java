...
public class CandleList {
	...
	/**
	 * ќтсекает голову у списка свечей до даты включительно
	...
	/**
	 * ќтсекает хвост у списка свечей до даты включительно
	 * @param toDate дата тоже отсекаетс€
	 */
	...
	public static Date closestCandleStartTime(Date toDate, CandleType candleType) {
		...
		if (DateUtils.addSeconds(result, candleType.getSeconds()).after(DateUtils.ceiling(result, candleType.getCalendarBase())) ) {
			...
			// т.к., 21:47:55.000 еще одной свечей дополнить нельз€
		...
		synchronized (map) {
			...
			if (tick.getTime().after(newTime)) {
				...
				if (topCandle != null) {
					...
					// если стара€ свеча закрыта, и в базу большей свечей не впихнуть
					// имеет смысл дл€ некратных базе размеров свечи типа 55 или 59 секунд
